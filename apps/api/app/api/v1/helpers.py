...
async def get_or_create_color_range(db: AsyncSession, name: str):
    color_range = db.query(models.ColorRange).filter(models.ColorRange.name == name).first()
    if not color_range:
...
async def get_or_create_product_type(db: AsyncSession, name: str):
    product_type = db.query(models.ProductType).filter(models.ProductType.name == name).first()
    if not product_type:
...
async def get_or_create_analogous(db: AsyncSession, value: str):
    analogous = db.query(models.Analogous).filter(models.Analogous.value == value).first()
    if not analogous:
